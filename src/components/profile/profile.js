...
function Profile(params) {
  ...
  async function getdata() {
    try {
      ...
      const data = await userServe.getUserById(userId);
  ...
  return (
    <div className="container" style={{ height: "80vh" }}>
      ...
      <div className="row justify-content-md-center h-100 align-items-center">
        <div className="col-12 col-md-5 col-lg-5 shadow p-md-4">
          ...
          <p className="h5 fw-bold mt-4">
            Mobile:{" "}
            <span className="text-info">{profileData?.mobileNumber}</span>
          </p>
          <p className="h5 fw-bold mt-4">
          ...
          <p className="h5 fw-bold mt-4">
            CreatedAt:{" "}
            <span className="text-info">{profileData?.createdAt}</span>
          </p>
        </div>
